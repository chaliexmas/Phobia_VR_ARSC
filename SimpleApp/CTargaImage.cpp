...
bool CTargaImage::Load(const char *filename){
	...
	//check the endiannes of the system
	long int i=1;
	const char *p= (const char *) &i;

	//Is this system use big endian
	if(p[0]!=1){
		std::cout<<"Big endian"<<std::endl;
		//tgaHeight = SwapTwoBytes(tgaHeight);
	}
...
/*2 byte Convert from little endian to big endian
WORD CTargaImage::SwapTwoBytes(WORD w){
      register WORD tmp;
      tmp =  (w & 0x00FF);
...
/*4 byte Convert from little endian to big endian
DWORD CTargaImage::SwapFourBytes(DWORD dw){
      register DWORD tmp;
      tmp =  (dw & 0x000000FF);
