...
void Dynamo::Init(void){
	...
	image->Load("rock.tga");
	glGenTextures(4, texs->texRock);
	glBindTexture(GL_TEXTURE_2D, (texs->texRock[0]));
	gluBuild2DMipmaps(GL_TEXTURE_2D, GL_RGB, image->GetWidth(), image->GetHeight(), GL_RGB, GL_UNSIGNED_BYTE, image->GetImage());
	image->Release();
	
	image->Load("sky.tga");
	glBindTexture(GL_TEXTURE_2D, (texs->texRock[1]));
	gluBuild2DMipmaps(GL_TEXTURE_2D, GL_RGB, image->GetWidth(), image->GetHeight(), GL_RGB, GL_UNSIGNED_BYTE, image->GetImage());
	image->Release();
	
	image->Load("roof.tga");
	glBindTexture(GL_TEXTURE_2D, (texs->texRock[2]));
	gluBuild2DMipmaps(GL_TEXTURE_2D, GL_RGB, image->GetWidth(), image->GetHeight(), GL_RGB, GL_UNSIGNED_BYTE, image->GetImage());
	image->Release();
	
	image->Load("roof.tga");
	glBindTexture(GL_TEXTURE_2D, (texs->texRock[3]));
	gluBuild2DMipmaps(GL_TEXTURE_2D, GL_RGB, image->GetWidth(), image->GetHeight(), GL_RGB, GL_UNSIGNED_BYTE, image->GetImage());
	image->Release();
...
void Dynamo::Fog(){
	...
	glPushMatrix();
	glFogi(GL_FOG_START, 52);//where the fog starts in front of you
...
/**Adjust the walls and floor**/
void Dynamo::setDir(float alpha, float omega, float beta){
	//adjust path size
	pathsize=alpha;
	
	//adjust left wall
	lm_angle = (float)(pathsize+4.0)* cosf(beta);
	lp_angle = (float)(pathsize+4.0)* sinf(beta);
	
	//adjust right wall
	rm_angle = (float)(pathsize+4.0)* cosf(omega);
	rp_angle = (float)(pathsize+4.0)* sinf(omega);
}
